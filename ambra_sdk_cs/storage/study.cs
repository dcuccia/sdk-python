...
using static AmbraSdk.Storage.ConverterHelpers;
...
{ 
    def validate_size_x_string(size_str: str):
        ...
        try {
            ...
            throw new ValueError("Wrong size format")

        try {
            int(width)
        catch (ValueError:
            throw new ValueError("Wrong width value")
        try {
            int(height)
        catch (ValueError:
            throw new ValueError("Wrong height value")
    ...
    public class Study:
        ...
        def thumbnail(
            ...
            if depth not in {8, 16}:
                throw new ValueError("Depth must be in (8, 16)")
        ...
        def download(
            ...
            if bundle not in {"dicom", "iso", "osx", "win"}:
                throw new ValueError("Boundle not in ("dicom", "iso", "osx", "win")")
